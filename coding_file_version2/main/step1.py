    ...
    def set_pexpect_command(self,json_path,json_file,log_path):
        ...
        #     print("{0}:\n{1}",("res",res))
            ...
            if not False:
                ...
                # for i in jsonpath.jsonpath(data, "$.body[?(@.sendline)]"):
                #     # print("{0}:\t{1}".format("i", i))
                #     try:
                #         HU.pAction_v2(i,cls= p, logFile=sys.stdout)
                #     except pexpect.TIMEOUT:
                #         # print("yes")
                #         print(p.before, p.after)
                # for i in jsonpath.jsonpath(data, "$.tail[?(@.sendline)]"):
                #     # print("{0}:\t{1}".format("i", i))
                #     try:
                #         HU.pAction_v2(i, cls=p, logFile=sys.stdout)
                #     except pexpect.TIMEOUT:
